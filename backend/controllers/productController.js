...
exports.getRelatedProducts = async (req, res) => {
   try {
      ...
      const products = await Product.find(query).limit(parseInt(limit)).populate('category');
...
exports.createProduct = async (req, res) => {
   try {
      const { name, description, price, categoryId, stock, sizes, colors, sizeGuideType } =
         req.body;
      let image = '';
      let detailImages = [];
      ...
      const newProduct = new ProductBuilder()
         ...
         .setStock(stock)
         .setSizes(sizes ? sizes.split(',').map((size) => size.trim()) : [])
         .setColors(colors ? colors.split(',').map((color) => color.trim()) : [])
